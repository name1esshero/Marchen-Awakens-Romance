...
*   **The "Shiftable" Decomp and xMAP files:** in more advanced decomp projects (e.g. pmd-sky), the whole codebase is "shiftable" -- every pointer is a linker symbol rather than a hardcoded address, so code can be added or removed without breaking anything. Compatibility with old ASM patches that assumed fixed addresses is then managed with xMAP files that calculate and apply the resulting pointer shifts. Not directly applicable to a project still mid-decompilation with `AT()`-pinned addresses, but the underlying principle -- avoid hardcoded addresses in C, always go through a symbol -- is exactly what the `#define`/`.set` aliasing techniques in this document are working toward.
