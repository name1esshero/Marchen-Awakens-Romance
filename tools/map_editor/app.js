...
let map=null,script=null,selectedTile=0,dirty=false,busy=false,stroke=null,eventDrag=null,visible=[],undo=[],redo=[],atlas=new Map(),catalog=null,mode='map';
...
function restore(s){map.document=s.document;if(script&&s.script===script.name&&s.events)script.document=s.events;playback.running=false;playback.active=false;cancelAnimationFrame(playback.request);drawMap();drawArgs();dirty=true;controls();}
...
function drawInitialSprites(ctx,zoom) {
    ...
    if(playback.active&&playback.actors.length){drawPlaybackSprites(ctx,zoom);return;}
    for (const source of script.sprite_placements||[]) {
        const item=currentSpritePlacement(source);
        ...
        ctx.save();
        const initCall=script.calls.find(call=>call.offset===item.init_offset);
        if(initCall===selectedCall()){ctx.strokeStyle='#fff36b';ctx.lineWidth=2/zoom;ctx.strokeRect(item.x+preview.x,item.y+preview.y,preview.width,preview.height);}
...
function paint(event,pick=false){if(!map||busy||!['map','collision'].includes(mode))return;const p=point(event);if(!p)return;const target=layer(),word=target.entries[p.index];$('position').textContent=`Tile (${p.x}, ${p.y}) · pixel (${p.x*8}, ${p.y*8}) · value ${word} / 0x${word.toString(16).padStart(4,'0')}`;if(pick){if(mode==='collision')$('attribute').value=word;else{if((word&1023)>=map.tiles.length){const resolution=map.resolved_tiles?.[word&1023];status(resolution?`Tile ${word&1023} is a ${resolution.confidence} transparent runtime tile; choose stored artwork to replace it.`:'This tile reference is unresolved. Choose a decoded tile to replace it.',!resolution);return;}selectedTile=word&1023;$('palette').value=word>>>12;$('hflip').checked=!!(word&1024);$('vflip').checked=!!(word&2048);drawTiles();}return;}if(!stroke)return;let value;if(mode==='collision'){value=Number($('attribute').value);const max=target.word_size===1?255:65535;if(!Number.isInteger(value)||value<0||value>max){status(`Attribute must be an integer from 0 to ${max}.`,true);return;}}else value=selectedTile|(bank()<<12)|($('hflip').checked?1024:0)|($('vflip').checked?2048:0);if(word!==value){target.entries[p.index]=value;stroke.changed=true;dirty=true;drawMap();controls();}}
...
$('map').addEventListener('pointerdown',e=>{if(e.button!==0||!map||busy)return;if(mode==='events'){beginEventDrag(e);return;}if(!['map','collision'].includes(mode))return;stroke={before:state(),changed:false};$('map').setPointerCapture(e.pointerId);paint(e);});
...
function setMode(next){if(next==='collision'&&map&&!map.document.attributes){status('This map has no decoded attribute plane.',true);return;}mode=next;document.body.dataset.mode=mode;document.querySelectorAll('#modes button').forEach(button=>button.classList.toggle('active',button.dataset.mode===mode));$('inspector-title').textContent={map:'Map tools',collision:'Collision',events:'Decoded events',connections:'Field connections'}[mode];if(mode==='collision')$('layer').value='attributes';else if(mode==='map'&&$('layer').value==='attributes')$('layer').value='0';drawMap();renderConnections();}
function drawArgs(){drawMap();const div=$('arguments');div.replaceChildren();if(!script||$('calls').value==='')return;const call=script.calls[Number($('calls').value)];if(!call)return;const title=document.createElement('p');title.textContent=`${call.function} (${call.count} arguments)`;div.append(title);const description=hitDescription(call);if(description){const note=document.createElement('p');note.textContent=description.note;div.append(note);}if(!call.editable){const values=call.decoded_arguments||[];values.forEach((arg,i)=>{const p=document.createElement('p');p.textContent=`Argument ${i}: ${arg.kind==='string'?'"'+arg.value+'"':arg.kind==='integer'?arg.value:'register '+arg.register+' (dynamic)'}`;div.append(p);});const p=document.createElement('p');p.textContent=values.length?'This call contains a string or dynamic expression; editing is disabled.':'Dynamic or unsupported argument sequence; editing is disabled.';div.append(p);return;}call.arguments.forEach((arg,i)=>{const label=document.createElement('label');label.textContent=`${description?.labels[i]||'Argument '+i} · CODE +0x${arg.offset.toString(16)}`;const input=document.createElement('input');input.type='number';input.min=-2147483648;input.max=2147483647;input.step=1;input.value=script.document.arguments[arg.offset]??arg.value;input.onchange=()=>{const value=Number(input.value);if(!Number.isInteger(value)||value<-2147483648||value>2147483647){status('Argument must be a signed 32-bit integer.',true);drawArgs();return;}remember();script.document.arguments[arg.offset]=value;drawArgs();status('Event argument changed. Save sources to write its JSON override.');};label.append(input);div.append(label);});}
async function loadScript(name){script=null;previewPlacements.clear();buildPlayback();renderScriptLinks();drawCalls();renderConnections();if(!name){$('script-info').textContent='No script selected.';return;}try{script=await api('script/'+encodeURIComponent(name));const a=script.analysis||{},counts={all:script.calls.length,sprite:0,hit:0,field:0,other:0};script.calls.forEach(call=>counts[eventClass(call)]++);const labels={all:'All decoded events',sprite:'Objects / sprites',hit:'Hit regions',field:'Field loads',other:'Other calls'};for(const option of $('event-class').options)option.textContent=`${labels[option.value]} (${counts[option.value]})`;$('script-info').textContent=`${name} · ${script.calls.filter(c=>c.editable).length}/${script.calls.length} literal calls editable · ${(a.field_loads||[]).length} field loads · ${(a.sprite_resources||[]).length} sprite resources · ${(a.sprite_properties||[]).length} sprite properties`;buildPlayback();renderScriptLinks();drawCalls();renderConnections();}catch(e){$('script-info').textContent='Read-only: '+e.message;status('Map is editable; this script could not be decoded: '+e.message,true);}}
async function loadMap(name){busy=true;controls();try{map=await api('map/'+encodeURIComponent(name));atlas=new Map();selectedTile=0;visible=map.document.planes.map(()=>true);$('maps').value=name;const d=map.document;$('dimensions').textContent=`${d.width} × ${d.height} tiles · ${d.width*8} × ${d.height*8} pixels`;options($('layer'),d.planes.map((p,i)=>[i,'Plane '+p.index]).concat(d.attributes?[['attributes','Raw attributes']]:[]));options($('palette'),Array.from({length:map.palette_banks},(_,i)=>[map.palette_base+i,String(map.palette_base+i)]));const field=$('visibility');field.replaceChildren();const legend=document.createElement('legend');legend.textContent='Visible layers';field.append(legend);d.planes.forEach((p,i)=>{const label=document.createElement('label'),input=document.createElement('input');input.type='checkbox';input.checked=true;input.onchange=()=>{visible[i]=input.checked;drawMap();};label.append(input,' Plane '+p.index);field.append(label);});dirty=false;undo=[];redo=[];drawTiles();drawMap();status(`Loaded ${name}. ${map.unresolved.length?map.unresolved.length+' unresolved cells are marked pink and preserved.':map.resolved_cells.length?map.resolved_cells.length+' external blank cells were resolved from documented runtime context.':'Sources are unchanged until you save.'}`);const associated=map.scripts[0]||'';$('scripts').value=associated;await loadScript(associated);}catch(e){status(e.message,true);}finally{busy=false;controls();}}
...
$('undo').onclick=undoEdit;$('redo').onclick=redoEdit;$('calls').onchange=drawArgs;$('calls').ondblclick=()=>{const call=selectedCall();if(call?.function==='FldSet')openDestination(decodedValues(call)[0]);};$('call-filter').oninput=drawCalls;$('event-class').onchange=drawCalls;$('palette').onchange=drawTiles;for(const id of ['zoom','grid','layer','game-order','show-sprites','show-hit-regions','hit-preview','sprite-guide'])$(id).onchange=drawMap;document.querySelectorAll('#modes button').forEach(button=>button.onclick=()=>setMode(button.dataset.mode));
