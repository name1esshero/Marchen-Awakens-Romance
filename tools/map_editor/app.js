...
function drawArgs(){drawMap();const div=$('arguments');div.replaceChildren();if(!script||$('calls').value==='')return;const call=script.calls[Number($('calls').value)];if(!call)return;const title=document.createElement('p');title.textContent=`${call.function} (${call.count} arguments)`;div.append(title);if(!call.editable){const p=document.createElement('p');p.textContent='Dynamic or unsupported argument sequence; editing is disabled.';div.append(p);return;}const description=hitDescription(call);if(description){const note=document.createElement('p');note.textContent=description.note;div.append(note);}call.arguments.forEach((arg,i)=>{const label=document.createElement('label');label.textContent=`${description?.labels[i]||'Argument '+i} · CODE +0x${arg.offset.toString(16)}`;const input=document.createElement('input');input.type='number';input.min=-2147483648;input.max=2147483647;input.step=1;input.value=script.document.arguments[arg.offset]??arg.value;input.onchange=()=>{const value=Number(input.value);if(!Number.isInteger(value)||value<-2147483648||value>2147483647){status('Argument must be a signed 32-bit integer.',true);drawArgs();return;}remember();script.document.arguments[arg.offset]=value;drawArgs();status('Event argument changed. Save sources to write its JSON override.');};label.append(input);div.append(label);});}
...
$('undo').onclick=undoEdit;$('redo').onclick=redoEdit;$('calls').onchange=drawArgs;$('call-filter').oninput=drawCalls;$('palette').onchange=drawTiles;for(const id of ['zoom','grid','layer','hit-preview'])$(id).onchange=drawMap;
