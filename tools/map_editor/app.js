...
let map=null,script=null,mapEventScripts=[],pendingEventOffset=null,selectedTile=0,dirty=false,busy=false,stroke=null,eventDrag=null,visible=[],undo=[],redo=[],atlas=new Map(),catalog=null,mode='map',tool='pencil',marscriptDoc=null,scriptEditorDirty=false;
const spriteImages=new Map();
const connectionMapCache=new Map();
const previewPlacements=new Map();
let playback={active:false,running:false,frame:0,duration:0,actors:[],lastTime:0,unresolved:0,inherited:0,request:0};
...
function currentSpritePlacement(item,source=script){return {...item,x:source.document.arguments[item.x_argument_offset]??item.x,y:source.document.arguments[item.y_argument_offset]??item.y};}
...
function drawInitialSprites(ctx,zoom) {
    ...
    if(mode==='events'&&$('show-map-event-sources').checked)for(const source of mapEventScripts){
        ...
        for(const placement of source.data.sprite_placements||[]){
            const item=currentSpritePlacement(placement,source.data);
...
function inheritedActorOrigin(id){const origins=[];for(const source of map?.event_sources||[])for(const item of source.sprite_origins||[])if(item.sprite===id)origins.push({...item,script:source.script});const positions=new Map(origins.map(item=>[`${item.x},${item.y}`,item]));return positions.size===1?[...positions.values()][0]:null;}
function buildPlayback(){cancelAnimationFrame(playback.request);const candidates=new Map((script?.sprite_candidates||[]).map(item=>[item.offset,item])),placements=new Map((script?.sprite_placements||[]).map(item=>[item.init_offset,currentSpritePlacement(item)])),actors=new Map(),pending=new Map(),registers={};let time=0,unresolved=0;
    ...
    playback={active:false,running:false,frame:0,duration:Math.max(1,time,...pending.values()),actors:[...actors.values()],lastTime:0,unresolved,inherited,request:0};$('preview-time').max=String(playback.duration);$('preview-time').value='0';updatePlaybackClock();drawMap();renderSpriteCandidates();}
function updatePlaybackClock(){$('preview-time').value=String(Math.round(playback.frame));$('preview-clock').value=`${Math.round(playback.frame)} / ${playback.duration} frames${playback.inherited?' · '+playback.inherited+' inherited map origin'+(playback.inherited===1?'':'s'):''}${playback.unresolved?' · '+playback.unresolved+' unresolved steps skipped':''}`;}
...
function eventAt(event){if(!script)return null;const {x,y}=eventPoint(event);let best=null;
    const sources=[...mapEventScripts.filter(source=>source.data&&source.data.name!==script.name).map(source=>source.data),script];
    for(const data of sources)for(const placement of data.sprite_placements||[]){const item=currentSpritePlacement(placement,data),preview=item.preview,rx=item.x+(preview?.x||-8),ry=item.y+(preview?.y||-8),w=preview?.width||16,h=preview?.height||16;if(x>=rx&&x<=rx+w&&y>=ry&&y<=ry+h){const index=data.calls.findIndex(call=>call.offset===item.init_offset);if(index>=0)best={index,distance:0,type:'sprite',item,source:data};}}
...
function beginEventDrag(event){const target=eventAt(event);if(!target)return;playback.running=false;playback.active=false;cancelAnimationFrame(playback.request);if(target.source&&target.source.name!==script.name){pendingEventOffset=target.item.init_offset;$('scripts').value=target.source.name;$('scripts').dispatchEvent(new Event('change'));return;}selectCall(target.index);const pointer=eventPoint(event);let offsets,start;
...
$('scripts').onchange=async()=>{const name=$('scripts').value;if(dirty&&!confirm('Switching scripts clears undo history. Save first to keep unsaved event edits. Continue?')){pendingEventOffset=null;$('scripts').value=script?.name||'';return;}if(scriptEditorDirty&&!confirm('Switching scripts discards your unsaved script text. Save it first to keep it. Continue?')){pendingEventOffset=null;$('scripts').value=script?.name||'';return;}busy=true;controls();await loadScript(name);undo=[];redo=[];busy=false;controls();};
