...
followed by native call 80/u32. Only contiguous literal pushes are editable.
"""
import hashlib
import json
    ...
    while cursor+4<=len(raw):
        ...
        while pos<len(payload) and payload[pos]:
            ...
            while True:
                ...
                start=ref-count*5
                editable=0<=start and count<=64 and all(code[start+i*5]==0x29 for i in range(count))
