...
for(const g of data.groups){const b=document.createElement('button');b.textContent=g.name;b.onclick=()=>selectGroup(g);$('groups').append(b)}$('search').oninput=()=>{[...$('groups').children].forEach(b=>b.hidden=!b.textContent.toLowerCase().includes($('search').value.toLowerCase()))};$('animation').onchange=selectAnimation;$('zoom').onchange=show;$('play').onclick=()=>{if(timer){stop();return}$('play').textContent='Pause';timer=setInterval(()=>{index=(index+1)%animation.frames.length;show()},1000/Math.max(1,Math.min(60,Number($('fps').value)||8)))};selectGroup(data.groups[0]);</script></html>'''
