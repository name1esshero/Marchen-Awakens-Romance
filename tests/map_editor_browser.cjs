...
(async()=>{
 ...
 try{
  ...
  await page.goto(session.url);await page.waitForFunction(()=>typeof map!=='undefined'&&map&&!busy);
  await page.locator('#maps').selectOption('MAP01_A.KMP');
  await page.waitForFunction(()=>map?.name==='MAP01_A.KMP'&&!busy&&document.querySelector('#script-info').textContent.includes('literal calls editable'));
  ...
  await page.locator('#modes button[data-mode="connections"]').click();
  const destinationCard=page.locator('#outgoing-links .connection[data-destination="AD4_B_A.KMP"]');
  assert.equal(await destinationCard.count(),1);
  page.once('dialog',dialog=>dialog.accept());
  await destinationCard.dblclick();
  await page.waitForFunction(()=>map?.name==='AD4_B_A.KMP'&&!busy);
  assert.equal(await page.locator('#maps').inputValue(),'AD4_B_A.KMP');
  await page.locator('#maps').selectOption('MAP01_3A.KMP');
  await page.waitForFunction(()=>map?.name==='MAP01_3A.KMP'&&!busy);
  await page.locator('#modes button[data-mode="events"]').click();
  const dorothyOrigin=page.locator('#event-sources .event-source',{hasText:'EV_BA02.SPC'});
  assert.equal(await dorothyOrigin.count(),1);
  await dorothyOrigin.click();
  await page.waitForFunction(()=>script?.name==='EV_BA02.SPC'&&!busy);
  const origin=await page.evaluate(()=>script.sprite_placements.find(item=>item.resource==='09A00'));
  assert.deepEqual([origin.sprite,origin.x,origin.y],[2,530,308]);
  const dorothySource=page.locator('#event-sources .event-source',{hasText:'EV_BA03.SPC'});
  assert.equal(await dorothySource.count(),1);
  await dorothySource.click();
  await page.waitForFunction(()=>script?.name==='EV_BA03.SPC'&&!busy);
  assert((await page.locator('#sprite-candidates').textContent()).includes('Dorothy'));
  assert((await page.locator('#sprite-candidates').textContent()).includes('inherits (530, 308) from EV_BA02.SPC'));
  await page.locator('#preview-play').click();
  await page.waitForFunction(()=>playback.active&&playback.inherited===1);
