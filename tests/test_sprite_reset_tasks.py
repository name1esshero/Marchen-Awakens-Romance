...
struct ScriptSprite *sub_080106C8(s32 id){assert(id>=0&&id<32);return (struct ScriptSprite *)(pool.bytes+id*40);}
void sub_08008BD8(void *p){assert(p==auxiliary);torn++;}
...
int main(void){
 ...
 struct ScriptSprite *s=sub_080106C8(31);
 ...
 *(u16 *)((u8 *)s+26)=0;ScriptSpriteResetTask(task.bytes);
 ...
 sub_080106C8(1)->active=1;sub_080106C8(1)->x=100;
 s=sub_080106C8(3);s->active=1;*(u16 *)((u8 *)s+26)=2;
 sub_080106C8(5)->x=42; /* inactive slot must be left untouched by reset-all */
 ScriptSpriteResetAllTask(task.bytes);defaults(sub_080106C8(1));assert(completed==1&&result==77&&s->active&&sub_080106C8(5)->x==42);
 *(u16 *)((u8 *)s+26)=0;ScriptSpriteResetAllTask(task.bytes);defaults(s);assert(completed==2&&finished==2&&result==-1);
 ...
 s=sub_080106C8(31);s->active=0;s->x=7;*(u16 *)((u8 *)s+26)=1;*(s32 **)(task.bytes+24)=0;
 ScriptSpriteResetTask(task.bytes);defaults(s);assert(completed==3&&finished==3&&freed==1);
 return 0;
}
''')
   subprocess.run(['gcc','-O2','-fno-strict-aliasing','-I'+str(ROOT/'include'),str(p/'test.c'),'-o',str(p/'test')],check=True)
