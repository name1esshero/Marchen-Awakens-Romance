...
- Checked every range declared in `src/decompiled.json` against its linked
  object provider, ROM address, and section size. The 53 declared ranges
  comprise 43 compiled-C ranges and 10 BIOS inline-assembly wrappers. A range
...
Runtime lookup/fallback is host-tested. A full emulator playthrough is still
needed; other printers, pagination and context-specific lookups are unfinished.
